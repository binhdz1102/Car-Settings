...
internal fun VehicleControlsPane(
    ...
    val scrollModifier =
        if (scrollInternally) {
            Modifier
                .vehicleBScrollbar(controlsScrollState)
    ...
    Column(
        ...
        if (zones.size > 1) {
            VehicleZoneFocusSelector(
                ...
                areaId = zoneAreaId,
                previousAreaId = previewAreaId,
                nextAreaId = controlsAreaId,
...
internal fun VehicleZoneFocusSelector(
    ...
    FocusArea(
        id = FocusAreaId(areaId),
        firstFocusAt = actionableFocusIds.firstOrNull(),
        focusOrder = actionableFocusIds,
        previousFocusArea = FocusAreaId(previousAreaId),
        nextFocusArea = FocusAreaId(nextAreaId),
        modifier = Modifier.fillMaxWidth(),
    ) {
                ...
                zones.forEachIndexed { index, zone ->
                    FocusItem(
                        ...
                        BChip(
                            ...
                            style = if (state.isFocused) BChipStyle.Filled else BChipStyle.Outlined,
                            size = BChipSize.Lg,
                        ) {
                            Text(zone.label)
                        }
...
internal fun vehicleControlStateDescription(
    ...
    when {
        ...
        !control.readable -> disconnectedReason
            ...
            if (control.pending) {
                "$valueDescription, ${stringResource(R.string.vehicle_control_pending)}"
            } else {
                valueDescription
            }
        }
