...
/**
 * Reports when a real rotary item has materialized for a destination handoff.
 *
 * The callback is intentionally kept at the app boundary. B-Material owns the native focus
 * lifecycle, while SettingsScaffold uses this signal to keep a root FocusArea non-focusable until
 * its requested lazy target exists. That prevents the native FocusArea from choosing a temporary
 * fallback row while asynchronous screen data is still being loaded.
 */
internal val LocalSettingsFocusItemAvailabilityObserver =
    staticCompositionLocalOf<(FocusItemId, Boolean) -> Unit> { { _, _ -> } }
...
fun FocusItem(
    ...
    val onContentFocus = LocalSettingsContentFocusObserver.current
    val onAvailability = LocalSettingsFocusItemAvailabilityObserver.current
    if (LocalRotaryFallback.current || LocalRotaryFocusController.current == null) {
        ...
        BMaterialFocusItem(
            ...
            content = { state ->
                // FocusItemView is a native focusable View. Android's default focus highlight
                // otherwise paints a translucent rectangle over the renderer, which conflicts
                // with the app-wide border-only CCP presentation. The Compose renderer is hosted
                // by a child ComposeView, so walk to the B-Material parent and disable that
                // platform decoration once per item.
                ...
                SideEffect {
                    onAvailability(id, focusItemView != null && isEnabled)
                }
                DisposableEffect(id, onAvailability) {
                    onDispose { onAvailability(id, false) }
                }
                LaunchedEffect(id, state.isFocused, onContentFocus) {
                    if (state.isFocused) onContentFocus(id)
                }
                RotaryFocusContent(
                    state = state,
...
private fun View.findRotaryFocusItemView(): FocusItemView? {
    var parent: ViewParent? = parent
    while (parent != null) {
        if (parent is FocusItemView) return parent
