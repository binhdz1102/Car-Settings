...
/**
 * Pending writes are synchronization state, not user-facing copy. Keeping the value description
 * stable prevents a short-lived VHAL acknowledgement from changing row height or semantics.
 */
internal fun stableVehicleValueStateDescription(valueDescription: String): String = valueDescription

/**
 * Returns only durable availability/read-only annotations. A pending write must never add a
 * transient line to a vehicle row.
 */
internal fun stableVehicleControlAnnotation(
    enabled: Boolean,
    readOnly: Boolean,
    ...
    when {
        !enabled -> disabledReason ?: unavailableDescription
