...
internal fun VehicleFeatureHostlessFallback(
    ...
    val fallbackFocusAreaIds =
        when {
            loading && controls.isEmpty() -> emptyList()
            !connected && visibleControls.isEmpty() -> listOf("vehicle-fallback-unavailable")
            visibleControls.isEmpty() -> listOf("vehicle-fallback-empty")
            visualization != null -> listOf("vehicle-fallback-preview-area")
            else -> emptyList()
        }
    ...
    VehicleSettingsScaffold(
        ...
        modifier = modifier,
        contentFocusAreaIds = fallbackFocusAreaIds,
    ) { padding ->
        when {
                ...
                LazyColumn(
                    modifier =
                        Modifier
                            .fillMaxSize()
                            .padding(padding)
                            .automotiveLazyScrollbar(fallbackListState),
