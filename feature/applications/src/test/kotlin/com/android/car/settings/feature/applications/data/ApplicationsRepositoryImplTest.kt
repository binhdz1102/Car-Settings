...
private class RecordingApplicationsPlatform : ApplicationsPlatform {
    ...
    override val unusedApps = MutableStateFlow<List<ApplicationSummary>>(emptyList())
    ...
    override val permissionGroups =
        MutableStateFlow<List<com.android.car.settings.feature.applications.domain.PermissionGroupSummary>>(emptyList())
    override val permissionApps = MutableStateFlow<List<ApplicationSummary>>(emptyList())
    override val selectedAppPermissions =
        MutableStateFlow<List<com.android.car.settings.feature.applications.domain.AppPermissionState>>(emptyList())
    override val defaultAppRoles = MutableStateFlow<List<com.android.car.settings.feature.applications.domain.DefaultAppRole>>(emptyList())
    override val openingLinks = MutableStateFlow<List<com.android.car.settings.feature.applications.domain.OpeningLinkState>>(emptyList())
    ...
    override suspend fun selectPermissionGroup(groupName: String) = record("permission-group:$groupName")

    override suspend fun selectApplicationPermissions(packageName: String) = record("permission-app:$packageName")

    override suspend fun setPermission(
        packageName: String,
        permissionName: String,
        granted: Boolean,
    ) = record("permission:$packageName:$permissionName:$granted")

    override suspend fun refreshDefaultApps() = record("default-refresh")

    override suspend fun setDefaultApp(
        roleName: String,
        packageName: String,
    ) = record("default:$roleName:$packageName")

    override suspend fun refreshOpeningLinks() = record("links-refresh")

    override suspend fun setOpeningLinksEnabled(
        packageName: String,
        enabled: Boolean,
    ) = record("links:$packageName:$enabled")
