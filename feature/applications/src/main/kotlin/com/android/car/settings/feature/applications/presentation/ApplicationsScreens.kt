...
private fun ApplicationsScreen(
    ...
    SettingsScaffold(
        ...
        isRoot = true,
        onBack = onBack,
        firstContentFocusId = APPLICATIONS_ALL_APPS_FOCUS_ID,
        lazyListState = listState,
        lazyFocusItemIndexById = applicationsRootLazyFocusIndexById(state.applications.recentApps),
        actions = {
        ...
        LazyColumn(
            ...
            item {
                SettingsActionRow(
                    navigates = true,
                    title = "All apps",
                    focusId = APPLICATIONS_ALL_APPS_FOCUS_ID,
            ...
            item {
                SettingsActionRow(
                    ...
                    focusId = APPLICATIONS_PERMISSIONS_FOCUS_ID,
            ...
            item {
                SettingsActionRow(
                    navigates = true,
                    title = "Default apps",
                    focusId = APPLICATIONS_DEFAULT_APPS_FOCUS_ID,
            ...
            item {
                SettingsActionRow(
                    navigates = true,
                    title = "Unused apps",
                    focusId = APPLICATIONS_UNUSED_APPS_FOCUS_ID,
            ...
            item {
                SettingsActionRow(
                    ...
                    focusId = APPLICATIONS_PERFORMANCE_APPS_FOCUS_ID,
            ...
            item {
                SettingsActionRow(
                    ...
                    focusId = APPLICATIONS_SPECIAL_ACCESS_FOCUS_ID,
...
fun AllApplicationsRoute(
    ...
    SettingsScaffold(
        ...
        onBack = onBack,
        lazyListState = listState,
        lazyFocusItemIndexById = allApplicationsLazyFocusIndexById(state.applications.apps),
        actions = {
            SettingsAppBarAction(
                id = "all-apps-refresh",
                contentDescription = "Refresh",
        ...
        LazyColumn(modifier = Modifier.fillMaxSize(), state = listState) {
            item {
                SettingsSwitchRow(
                    ...
                    focusId = ALL_APPLICATIONS_SHOW_SYSTEM_FOCUS_ID,
...
private fun ApplicationDetailsScreen(
    ...
    SettingsScaffold(
        ...
        destinationKey = "application-details",
        onBack = onBack,
        actions = {
            SettingsAppBarAction(
...
fun PermissionGroupApplicationsRoute(
    ...
    val title =
        state.selectedGroup
            ...
            ?: "Permission group"
    SettingsScaffold(title = title, destinationKey = "application-permission-group", onBack = onBack) {
        PermissionMessageHost(state.message, viewModel::clearMessage)
        LazyColumn(modifier = Modifier.fillMaxSize()) {
...
fun ApplicationPermissionsRoute(
    ...
    RefreshOnResume(viewModel::refresh)
    SettingsScaffold(
        title = state.permissions.firstOrNull()?.applicationLabel ?: "App permissions",
        destinationKey = "application-permissions-detail",
        onBack = onBack,
    ) {
        PermissionMessageHost(state.message, viewModel::clearMessage)
        LazyColumn(modifier = Modifier.fillMaxSize()) {
...
fun SpecialAccessListRoute(
    ...
    SettingsScaffold(
        ...
        destinationKey = "special-app-access-list",
        onBack = onBack,
        actions = {
            SettingsAppBarAction(
...
private fun ApplicationRow(
    ...
    SettingsActionRow(
        ...
        focusId = applicationFocusId(application.packageName),
        onClick = onClick,
    )
}

internal fun allApplicationsLazyFocusIndexById(applications: List<ApplicationSummary>): Map<String, Int> =
    linkedMapOf<String, Int>().apply {
        put(ALL_APPLICATIONS_SHOW_SYSTEM_FOCUS_ID, 0)
        applications.forEachIndexed { index, application ->
            put(applicationFocusId(application.packageName), index + 1)
        }
    }

/** Logical root Apps order mapped to its real LazyColumn slots, including section headers. */
internal fun applicationsRootLazyFocusIndexById(recentApplications: List<ApplicationSummary>): Map<String, Int> =
    linkedMapOf<String, Int>().apply {
        put(APPLICATIONS_ALL_APPS_FOCUS_ID, 1)
        var nextIndex = 2
        if (recentApplications.isNotEmpty()) {
            nextIndex += 1 // Recently opened section header.
            recentApplications.forEach { application ->
                put(applicationFocusId(application.packageName), nextIndex)
                nextIndex += 1
            }
        }
        nextIndex += 1 // App settings section header.
        put(APPLICATIONS_PERMISSIONS_FOCUS_ID, nextIndex++)
        put(APPLICATIONS_DEFAULT_APPS_FOCUS_ID, nextIndex++)
        put(APPLICATIONS_UNUSED_APPS_FOCUS_ID, nextIndex++)
        put(APPLICATIONS_PERFORMANCE_APPS_FOCUS_ID, nextIndex++)
        put(APPLICATIONS_SPECIAL_ACCESS_FOCUS_ID, nextIndex)
    }

private const val ALL_APPLICATIONS_SHOW_SYSTEM_FOCUS_ID = "all-apps-show-system"

private const val APPLICATIONS_ALL_APPS_FOCUS_ID = "applications-all-apps"
private const val APPLICATIONS_PERMISSIONS_FOCUS_ID = "applications-app-permissions"
private const val APPLICATIONS_DEFAULT_APPS_FOCUS_ID = "applications-default-apps"
private const val APPLICATIONS_UNUSED_APPS_FOCUS_ID = "applications-unused-apps"
private const val APPLICATIONS_PERFORMANCE_APPS_FOCUS_ID = "applications-performance-apps"
private const val APPLICATIONS_SPECIAL_ACCESS_FOCUS_ID = "applications-special-access"

private fun applicationFocusId(packageName: String): String = "application-$packageName"
