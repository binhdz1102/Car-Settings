...
private fun SoundScreen(
    ...
    SettingsScaffold(
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            item { SettingsSection("Sound selection") {} }
            items(state.sound.ringtones, key = { it.kind }) { selection ->
                SettingsActionRow(
                    title = selection.kind.title(),
                    summary = selection.title,
                    leading = { Icon(Icons.Default.VolumeUp, contentDescription = null) },
    ...
    ListItem(
        ...
        supportingContent = {
            Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                ...
                Text("$displayedValue/${volume.maximum}" + if (volume.isMuted) " · Muted" else "")
            }
        },
    )
    Divider()
...
@Composable
private fun RingerModeRow(
    selected: RingerMode,
    onSelected: (RingerMode) -> Unit,
) {
    ListItem(
        headlineContent = { Text("Ringer mode") },
        supportingContent = {
            Text("${selected.readableName()}. Tap an option to change the system ringer mode.")
        },
    )
    RingerMode.entries.forEach { mode ->
        ListItem(
            headlineContent = { Text(mode.readableName()) },
            leadingContent = {
...
fun RingtonePickerRoute(
    ...
    val selected = state.sound.ringtones.firstOrNull { it.kind == kind }?.uri
...
private fun RingtoneOptionRow(
    ...
    ListItem(
        ...
        supportingContent = {
            if (option.uri != null) {
                TextButton(onClick = { onPreview(option.uri) }) { Text("Preview") }
            }
        },
        leadingContent = {
            RadioButton(selected = selected, onClick = { onSelect(option.uri) })
        },
        modifier = Modifier.fillMaxWidth(),
    )
    Divider()
...
private fun RingerMode.readableName(): String =
