...
private fun SoundScreen(
    ...
    val focusSpec =
        soundRootFocusSpec(
            volumeFocusIds = state.sound.volumes.map { soundVolumeFocusId(it.id) },
            isRingerModeSupported = state.sound.isRingerModeSupported,
            isWorking = state.isWorking,
            ringtoneFocusIds = state.sound.ringtones.map { soundRingtoneFocusId(it.kind) },
        )
    SettingsScaffold(
        ...
        LazyColumn(
            ...
            items(state.sound.ringtones, key = { "sound-ringtone-${it.kind}" }) { selection ->
                SettingsActionRow(
                    ...
                    focusId = soundRingtoneFocusId(selection.kind),
...
/** Keeps root Sound handoff aligned with the sections and async volume-group rows. */
internal fun soundRootFocusSpec(
    volumeFocusIds: List<String>,
...
private fun soundRingtoneFocusId(kind: RingtoneKind): String = "sound-ringtone-$kind"
...
private fun VolumeRow(
    ...
    SettingsFormSlider(
        focusId = soundVolumeFocusId(volume.id),
