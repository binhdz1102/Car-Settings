...
@Singleton
internal class SystemRepositoryImpl @Inject constructor(
    ...
    override suspend fun setAutofillService(componentName: String?): ActionResult =
        platform.setAutofillService(componentName)

    override suspend fun setKeyboardEnabled(id: String, enabled: Boolean): ActionResult =
        platform.setKeyboardEnabled(id, enabled)

    override suspend fun setTextToSpeechEngine(packageName: String): ActionResult =
        platform.setTextToSpeechEngine(packageName)

    override suspend fun setTextToSpeechPlayback(speechRate: Int, pitch: Int): ActionResult =
        platform.setTextToSpeechPlayback(speechRate, pitch)

    override suspend fun speakTextToSpeechSample(): ActionResult = platform.speakTextToSpeechSample()

    override suspend fun setVehicleUnit(propertyId: Int, unitId: Int): ActionResult =
        platform.setVehicleUnit(propertyId, unitId)

    override suspend fun launchExternal(actionId: SystemExternalActionId): ActionResult =
        platform.launchExternal(actionId)

    override suspend fun restartSystem(): ActionResult = platform.restartSystem()

    override suspend fun resetNetwork(subscriptionId: Int?, eraseEsim: Boolean): ActionResult =
        platform.resetNetwork(subscriptionId, eraseEsim)
