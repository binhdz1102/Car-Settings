...
private fun SystemHomeScreen(
    ...
    val focusSpec = systemRootFocusSpec(state.settings, state.isWorking)
    SettingsScaffold(
        title = "System",
        destinationKey = "system",
        isRoot = true,
        onBack = onBack,
        ...
        LazyColumn(
            modifier = Modifier.fillMaxSize().testTag("system-settings-list"),
            state = listState,
        ) {
            item(key = "section-system") { SettingsSection("System") {} }
            state.settings.systemUpdate?.let { action ->
                item(key = "row-system-update") { ExternalActionRow(action, !state.isWorking, onExternal) }
            }
            item(key = "row-languages") {
                SettingsActionRow(
                    title = "Languages & input",
                    focusId = SYSTEM_LANGUAGES_FOCUS_ID,
            ...
            item(key = "row-units") {
                SettingsActionRow(
                    title = "Units",
                    focusId = SYSTEM_UNITS_FOCUS_ID,
                    summary =
                        if (state.settings.units.settings
                                .isEmpty()
                        ) {
            ...
            item(key = "row-datetime") {
                SettingsActionRow(
                    title = "Date & time",
                    focusId = SYSTEM_DATE_TIME_FOCUS_ID,
            ...
            item(key = "row-storage") {
                SettingsActionRow(
                    title = "Storage",
                    focusId = SYSTEM_STORAGE_FOCUS_ID,
            ...
            item(key = "section-about") { SettingsSection("About") {} }
            item(key = "row-about") {
                SettingsActionRow(
                    title = "About",
                    focusId = SYSTEM_ABOUT_FOCUS_ID,
            ...
            item(key = "section-reset") { SettingsSection("Reset") {} }
            item(key = "row-reset") {
                SettingsActionRow(
                    title = "Reset options",
                    focusId = SYSTEM_RESET_FOCUS_ID,
            ...
            state.settings.developerOptions?.let { action ->
                item(key = "section-advanced") { SettingsSection("Advanced") {} }
                item(key = "row-developer-options") { ExternalActionRow(action, !state.isWorking, onExternal) }
            }
            if (state.settings.systemExtras.isNotEmpty()) {
                item(key = "section-extras") { SettingsSection("Additional system settings") {} }
                items(state.settings.systemExtras, key = { "extra-${it.packageName}/${it.className}" }) { action ->
...
internal fun systemRootFocusSpec(
    settings: com.android.car.settings.feature.system.domain.SystemSettingsState,
    isWorking: Boolean,
): SettingsLazyFocusListSpec =
    settingsLazyFocusListSpec(
        buildList {
            add(SettingsLazyFocusListSlot()) // System section heading.
            settings.systemUpdate?.let { action ->
                add(SettingsLazyFocusListSlot(systemExternalActionFocusId(action), isEnabled = !isWorking))
            }
            add(SettingsLazyFocusListSlot(SYSTEM_LANGUAGES_FOCUS_ID))
            add(SettingsLazyFocusListSlot(SYSTEM_UNITS_FOCUS_ID))
            add(SettingsLazyFocusListSlot(SYSTEM_DATE_TIME_FOCUS_ID))
            add(SettingsLazyFocusListSlot(SYSTEM_STORAGE_FOCUS_ID))
            add(SettingsLazyFocusListSlot()) // About section heading.
            add(SettingsLazyFocusListSlot(SYSTEM_ABOUT_FOCUS_ID))
            add(SettingsLazyFocusListSlot(SYSTEM_LEGAL_FOCUS_ID))
            add(SettingsLazyFocusListSlot()) // Reset section heading.
            add(SettingsLazyFocusListSlot(SYSTEM_RESET_FOCUS_ID))
            settings.developerOptions?.let { action ->
                add(SettingsLazyFocusListSlot()) // Advanced section heading.
                add(SettingsLazyFocusListSlot(systemExternalActionFocusId(action), isEnabled = !isWorking))
            }
            if (settings.systemExtras.isNotEmpty()) {
                add(SettingsLazyFocusListSlot()) // Additional system settings heading.
                settings.systemExtras.forEach { action ->
                    add(SettingsLazyFocusListSlot(systemExternalActionFocusId(action), isEnabled = !isWorking))
                }
            }
        },
    ...
    SettingsScaffold(title = "Hardware info", onBack = onBack, actions = { RefreshAction(viewModel::refresh) }) {
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            item {
                KeyValueRow(
                    "Hardware revision",
                    state.settings.about.hardware.revision
                        .ifBlank { "Unavailable" },
                )
            }
        }
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            item {
                SettingsActionRow(
                    title = "Keyboard",
                    summary =
                        state.settings.languageInput.keyboards
                            .count { it.enabled }
            ...
            item {
                SettingsActionRow(
                    ...
                    summary =
                        state.settings.languageInput.textToSpeech.currentEngine
                            ?.label
                            ?: "No engine installed",
                    leading = { Icon(Icons.Default.Settings, null) },
...
fun SystemLanguageRoute(
    ...
    val pendingLanguage =
        state.settings.languageInput.availableLocales
    ...
    val visibleLocales =
        remember(state.settings.languageInput.availableLocales, query) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            item {
                SettingsFormTextField(
                    focusId = "system-language-search",
            ...
            items(visibleLocales, key = { it.languageTag }) { locale ->
                SettingsActionRow(
                    title = locale.name,
                    focusId = "system-locale-${locale.languageTag}",
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            items(autofill.candidates, key = { it.componentName }) { candidate ->
                SettingsActionRow(
                    ...
                    focusId = "system-autofill-${candidate.componentName}",
                    summary =
                        if (candidate.componentName ==
                            autofill.currentService?.componentName
                        ) {
                            "Current selection"
                        } else {
...
fun KeyboardRoute(
    ...
    val pendingKeyboard =
        state.settings.languageInput.keyboards
            .firstOrNull { it.id == pendingKeyboardId }
    val snackbar = messageSnackbar(state.message, viewModel::clearMessage)
    ...
    SettingsScaffold(title = "Keyboard", onBack = onBack, actions = { RefreshAction(viewModel::refresh) }) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            if (state.settings.languageInput.keyboards
                    .isEmpty()
            ) {
                ...
                items(state.settings.languageInput.keyboards, key = { it.id }) { keyboard ->
                    SettingsSwitchRow(
                        ...
                        focusId = "system-keyboard-${keyboard.id}",
                        summary =
                            keyboard.summary.ifBlank {
...
fun TextToSpeechRoute(
    ...
    if (chooseEngine) {
        AutomotiveAlertDialog(
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    tts.engines.forEach { engine ->
                        OutlinedButton(
                            onClick = {
                                viewModel.setTextToSpeechEngine(engine.packageName)
                                chooseEngine = false
                            },
                            enabled = !state.isWorking,
            ...
            confirmAction = { chooseEngine = false },
        )
    }
    SettingsScaffold(title = "Text-to-speech output", onBack = onBack, actions = { RefreshAction(viewModel::refresh) }) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            item {
                SettingsFormButton(
                    focusId = "tts-reset-playback",
                    label = "Reset speech rate and pitch",
                    enabled = tts.currentEngine != null && !state.isWorking,
...
fun UnitsRoute(
    ...
    val selected =
        state.settings.units.settings
            .firstOrNull { it.propertyId == selectedPropertyId }
    val snackbar = messageSnackbar(state.message, viewModel::clearMessage)
    if (selected != null) {
        AutomotiveAlertDialog(
            ...
            confirmAction = { selectedPropertyId = null },
        )
    }
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            if (state.settings.units.settings
                ...
                items(state.settings.units.settings, key = { it.propertyId }) { setting ->
                    SettingsActionRow(
                        title = setting.title,
                        focusId = "system-unit-${setting.propertyId}",
...
fun StorageRoute(
    ...
    SettingsScaffold(title = "Storage", onBack = onBack, actions = { RefreshAction(viewModel::refresh) }) {
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            item {
                HintRow(
                    "Music & audio, Other apps, Files and System use the same per-user volume accounting categories as AAOS Settings.",
                )
            }
        }
    }
...
fun ResetNetworkRoute(
    ...
    var selectedSubscriptionId by remember(state.settings.reset.subscriptions) {
        mutableStateOf(
            state.settings.reset.subscriptions
                .firstOrNull()
                ?.id,
        )
    }
    ...
    SettingsScaffold(title = "Reset network settings", onBack = onBack, actions = { RefreshAction(viewModel::refresh) }) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            item {
                SettingsFormButton(
                    focusId = "reset-network-settings",
                    label = if (state.isWorking) "Resetting settings…" else "Reset settings",
...
fun ResetAppPreferencesRoute(
    ...
    if (confirm) {
        ConfirmationDialog(
            title = "Reset app preferences?",
            text =
                "This re-enables disabled apps, notifications, default-app actions and app operation modes. App data will not be deleted.",
    ...
    SettingsScaffold(title = "Reset app preferences", onBack = onBack, actions = { RefreshAction(viewModel::refresh) }) {
        ...
        Column(
            ...
            Text(
                "No app data will be deleted. Disabled apps, notifications, default applications " +
                    "and background restrictions will be restored to their defaults.",
            )
            SettingsFormButton(
                focusId = "reset-app-preferences",
                label = if (state.isWorking) "Resetting app preferences…" else "Reset app preferences",
...
fun FactoryResetRoute(
    ...
    SettingsScaffold(title = "Factory reset", onBack = onBack, actions = { RefreshAction(viewModel::refresh) }) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize(), state = listState) {
            ...
            if (state.settings.reset.accountsAffected
                    .isNotEmpty()
            ) {
            ...
            item {
                SettingsFormButton(
                    focusId = "factory-reset",
                    label = if (state.isWorking) "Erasing data…" else "Erase all data",
                    onClick = { firstConfirm = true },
...
private fun ExternalActionRow(
    ...
    title = action.title,
    focusId = systemExternalActionFocusId(action),
...
private fun systemExternalActionFocusId(action: SystemExternalAction): String =
    when (action.id) {
        SystemExternalActionId.SYSTEM_UPDATE -> SYSTEM_UPDATE_FOCUS_ID
        SystemExternalActionId.DEVELOPER_OPTIONS -> "system-developer-options"
        else -> "system-external-${action.id}-${(action.packageName + '/' + action.className).hashCode().toUInt().toString(16)}"
    }

private const val SYSTEM_UPDATE_FOCUS_ID = "system-update"
private const val SYSTEM_LANGUAGES_FOCUS_ID = "system-languages"
private const val SYSTEM_UNITS_FOCUS_ID = "system-units"
private const val SYSTEM_DATE_TIME_FOCUS_ID = "system-date-time"
private const val SYSTEM_STORAGE_FOCUS_ID = "system-storage"
private const val SYSTEM_ABOUT_FOCUS_ID = "system-about"
private const val SYSTEM_LEGAL_FOCUS_ID = "system-legal"
private const val SYSTEM_RESET_FOCUS_ID = "system-reset"
...
private fun RefreshAction(onRefresh: () -> Unit) {
    SettingsAppBarAction(
        id = "system-refresh",
        contentDescription = "Refresh system settings",
        onClick = onRefresh,
    ) {
...
private fun ConfirmationDialog(
    ...
    confirmButton = { Button(onClick = onConfirm) { Text(confirmLabel, style = MaterialTheme.typography.titleMedium) } },
    confirmAction = onConfirm,
    dismissAction = onDismiss,
    dismissButton = { OutlinedButton(onClick = onDismiss) { Text("Cancel", style = MaterialTheme.typography.titleMedium) } },
)
