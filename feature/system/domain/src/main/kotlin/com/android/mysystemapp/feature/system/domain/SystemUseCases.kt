...
class SystemUseCases @Inject constructor(
    ...
    suspend fun setKeyboardEnabled(id: String, enabled: Boolean) = repository.setKeyboardEnabled(id, enabled)

    suspend fun setTextToSpeechEngine(packageName: String) = repository.setTextToSpeechEngine(packageName)

    suspend fun setTextToSpeechPlayback(speechRate: Int, pitch: Int) =
    ...
    suspend fun setVehicleUnit(propertyId: Int, unitId: Int) = repository.setVehicleUnit(propertyId, unitId)
    ...
    suspend fun resetNetwork(subscriptionId: Int?, eraseEsim: Boolean) =
