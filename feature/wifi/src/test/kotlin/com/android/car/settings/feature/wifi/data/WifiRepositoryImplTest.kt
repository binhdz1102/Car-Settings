...
class WifiRepositoryImplTest {
    ...
    fun `repository delegates every platform operation`() =
        runTest {
            ...
            repository.setCellularFallbackEnabled(true)
            repository.setPersistentTetheringEnabled(true)
            repository.setMobileDataEnabled(true)
            repository.setDataRoamingEnabled(false)
            repository.setDefaultDataSubscription(2)

            assertThat(platform.calls).hasSize(19)
...
private class FakeWifiPlatform : WifiPlatform {
    ...
    override suspend fun setCellularFallbackEnabled(enabled: Boolean) = result("cellularFallback")

    override suspend fun setPersistentTetheringEnabled(enabled: Boolean) = result("persistentTethering")

    override suspend fun setMobileDataEnabled(enabled: Boolean) = result("mobileData")

    override suspend fun setDataRoamingEnabled(enabled: Boolean) = result("roaming")

    override suspend fun setDefaultDataSubscription(subscriptionId: Int) = result("defaultData")
