...
class WifiUseCases
    ...
    constructor(
        ...
        suspend fun setCellularFallbackEnabled(enabled: Boolean) = repository.setCellularFallbackEnabled(enabled)

        suspend fun setPersistentTetheringEnabled(enabled: Boolean) = repository.setPersistentTetheringEnabled(enabled)

        suspend fun setMobileDataEnabled(enabled: Boolean) = repository.setMobileDataEnabled(enabled)

        suspend fun setDataRoamingEnabled(enabled: Boolean) = repository.setDataRoamingEnabled(enabled)

        suspend fun setDefaultDataSubscription(subscriptionId: Int) = repository.setDefaultDataSubscription(subscriptionId)
    }
