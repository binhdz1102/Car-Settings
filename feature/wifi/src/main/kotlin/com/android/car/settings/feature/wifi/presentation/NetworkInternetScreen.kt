...
fun NetworkInternetRoute(
    ...
    val focusSpec =
        networkInternetRootFocusSpec(
            hotspotEnabled = !state.isWorking && !state.wifi.hotspot.isTransitioning,
            mobileNetworkEnabled = mobile.isSupported && mobile.subscriptions.isNotEmpty() && !state.isWorking,
            wifiEnabled =
                !state.isWorking &&
                    state.wifi.radioState != WifiRadioState.ENABLING &&
                    state.wifi.radioState != WifiRadioState.DISABLING,
            connectedNetworkFocusId = state.wifi.connectedNetwork?.let { "network-connected-${it.key}" },
        )
    SettingsScaffold(
...
/** Keeps root Network & internet handoff aligned with conditional connection rows. */
internal fun networkInternetRootFocusSpec(
