...
fun ClimateControl.displayValue(): String = when (capability.kind) {
    ClimateControlKind.TOGGLE -> if (booleanValue == true) "On" else "Off"
    ClimateControlKind.FLOAT_RANGE,
    ClimateControlKind.READ_ONLY_FLOAT -> floatValue?.let { "%.1f".format(it) } ?: "--"
    ClimateControlKind.INT_RANGE,
    ClimateControlKind.INT_OPTIONS -> intValue?.toString() ?: "--"
}
