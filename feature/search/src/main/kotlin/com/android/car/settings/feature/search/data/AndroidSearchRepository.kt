 ...
 */
@Singleton
internal class AndroidSearchRepository
    @Inject
    constructor(
        @ApplicationContext private val context: Context,
        private val vehicleUxPolicy: VehicleUxPolicy,
    ) : SearchRepository {
        ...
        private fun availabilityFor(destination: SearchDestination): Availability =
            if (
                vehicleUxPolicy.state.value !is VehicleUxPolicyState.Unrestricted &&
                !DefaultSettingsRegistry.definition(destination).uxSafetyClass.isAllowedWhileRestricted()
            ) {
                unavailable(context.getString(R.string.search_parked_only_reason))
            } else {
                capabilityAvailabilityFor(destination)
            }

        private fun capabilityAvailabilityFor(destination: SearchDestination): Availability =
            when (destination) {
                SearchDestination.WIFI,
                SearchDestination.WIFI_HOTSPOT,
                SearchDestination.WIFI_PREFERENCES,
                SearchDestination.NETWORK_INTERNET,
                SearchDestination.WIFI_QR,
                ->
                    if (packageManager.hasSystemFeature(PackageManager.FEATURE_WIFI)) {
                        available
                    } else {
                        unavailable("Wi-Fi hardware is not available on this vehicle")
                    }
                ...
                SearchDestination.BLUETOOTH ->
                    if (packageManager.hasSystemFeature(PackageManager.FEATURE_BLUETOOTH)) {
                        ...
                        unavailable("Bluetooth hardware is not available on this vehicle")
                    }

                SearchDestination.MOBILE_NETWORK ->
                    if (packageManager.hasSystemFeature(PackageManager.FEATURE_TELEPHONY)) {
                        available
                    } else {
                        unavailable("Mobile network hardware is not available on this vehicle")
                    }

                SearchDestination.PRIVACY_MICROPHONE,
                SearchDestination.PRIVACY_CAMERA,
                ->
                    if (hasPermission(PERMISSION_OBSERVE_SENSOR_PRIVACY)) {
                        available
                    } else {
                        unavailable(SENSOR_PRIVACY_ROLE_REQUIRED_MESSAGE)
                    }
                ...
                SearchDestination.SYSTEM_UI_COCKPIT,
                SearchDestination.SYSTEM_UI_NOTIFICATION_CENTER,
                SearchDestination.SYSTEM_UI_USER_CENTER,
                SearchDestination.SYSTEM_UI_CATALOG,
                SearchDestination.LAUNCHER,
                -> unavailable("Not included in the Car-Settings build")
...
private data class IndexableSetting(
    ...
    fun toResult(availability: Availability) =
...
private fun SettingsSearchResult.searchableText(): String =
    normalize(
...
private val catalog =
    listOf(
        IndexableSetting(
            "display",
            "Display",
            "Brightness, adaptive brightness and theme",
            "Display",
            "screen theme night day calm",
            SearchDestination.DISPLAY,
        ),
        IndexableSetting(
            "brightness",
            "Brightness",
        ...
        IndexableSetting(
            "time_zone",
            "Time zone",
            "Choose the vehicle time zone",
            "Date & time",
            "clock region",
            SearchDestination.TIME_ZONE,
        ),
        IndexableSetting(
            "network_internet",
            "Network & internet",
            "Wi-Fi, hotspot and mobile data",
            "Network & internet",
        ...
        IndexableSetting(
            "mobile_network",
            "Mobile network",
            "Mobile data, roaming and SIMs",
            "Network & internet",
            "cellular data sim roaming",
            SearchDestination.MOBILE_NETWORK,
        ),
        ...
        IndexableSetting(
            "wifi_hotspot",
            "Wi-Fi hotspot",
            "Share this vehicle's internet connection",
            "Wi-Fi",
            "tether tethering access point",
            SearchDestination.WIFI_HOTSPOT,
        ),
        IndexableSetting(
            "wifi_preferences",
            "Wi-Fi preferences",
            "Scanning, wakeup and network notifications",
            "Wi-Fi",
            "wifi scanning wakeup",
            SearchDestination.WIFI_PREFERENCES,
        ),
        IndexableSetting(
            "bluetooth",
            "Bluetooth",
            "Pair and manage connected devices",
            "Bluetooth",
        ...
        IndexableSetting(
            ...
            "Device visibility",
            "Make this vehicle discoverable",
            "Bluetooth",
            "discoverable",
            SearchDestination.BLUETOOTH,
        ),
        IndexableSetting(
            "sound",
            "Sound & vibration",
            "Volume, ringtones and Do Not Disturb",
            "Sound & vibration",
            "audio ringer vibration",
            SearchDestination.SOUND,
        ),
        IndexableSetting(
            "volume",
            "Volume",
            "Media, call, ring, notification and alarm volume",
            "Sound & vibration",
            "audio slider",
            SearchDestination.SOUND,
        ),
        ...
        IndexableSetting(
            "phone_ringtone",
            "Phone ringtone",
            "Choose the incoming call sound",
            "Sound & vibration",
            "call ring",
            SearchDestination.PHONE_RINGTONE,
        ),
        IndexableSetting(
            "notification_sound",
            ...
            "Sound & vibration",
            "alert tone",
            SearchDestination.NOTIFICATION_RINGTONE,
        ),
        IndexableSetting(
            "alarm_sound",
            "Default alarm sound",
            ...
            "Sound & vibration",
            "clock tone",
            SearchDestination.ALARM_RINGTONE,
        ),
        IndexableSetting(
            "do_not_disturb",
            "Do Not Disturb",
            "Control interruption filtering",
            "Sound & vibration",
            "dnd quiet",
            SearchDestination.SOUND,
        ),
        IndexableSetting(
            "climate",
            "Climate",
            "Temperature, airflow, defrost and seat comfort",
            "Climate",
            "hvac air conditioning fan seat",
            SearchDestination.HVAC,
        ),
        IndexableSetting(
            "climate_temperature",
            "Climate temperature",
            "Set temperature by vehicle zone",
            "Climate",
            "hvac driver passenger celsius fahrenheit",
            SearchDestination.HVAC,
        ),
        IndexableSetting(
            "climate_airflow",
            "Climate airflow",
            "Fan speed, direction, A/C and recirculation",
            "Climate",
            "hvac fan ac defrost",
            SearchDestination.HVAC,
        ),
        IndexableSetting(
            "driver_assistance",
            "Driver assistance",
            "Collision, lane, monitoring, speed, parking and OEM assistance preferences",
            "Vehicle control",
            "adas safety lane collision cruise parking",
            SearchDestination.DRIVER_ASSISTANCE,
        ),
        IndexableSetting(
            "seat_control",
            "Seats and steering wheel",
            "Position, support, memory, occupancy and steering adjustments",
            "Vehicle control",
            "seat lumbar headrest memory steering",
            SearchDestination.SEAT_CONTROL,
        ),
        IndexableSetting(
            "vehicle_lighting",
            "Vehicle lighting",
            "Exterior, hazard, cabin, reading and footwell lights",
            "Vehicle control",
            "headlight high beam fog cabin light",
            SearchDestination.VEHICLE_LIGHTING,
        ),
        IndexableSetting(
            "door_control",
            "Doors, windows and mirrors",
            "Locks, positions, movement, folding and mirror tilt",
            "Vehicle control",
            "door lock child window mirror fold",
            SearchDestination.DOOR_CONTROL,
        ),
        ...
        IndexableSetting(
            ...
            "Apps",
            "application list",
            SearchDestination.ALL_APPLICATIONS,
        ),
        IndexableSetting(
            "special_access",
            "Special app access",
            "Permissions with special access",
            "Apps",
            "overlay usage install unknown",
            SearchDestination.SPECIAL_APP_ACCESS,
        ),
        IndexableSetting(
            "performance_apps",
            "Performance impacting apps",
            "Apps affecting vehicle performance",
            "Apps",
            "hibernation unused",
            SearchDestination.PERFORMANCE_APPS,
        ),
        IndexableSetting(
            "notifications",
            "Notifications",
            "Recently sent notifications and app controls",
            "Notifications",
            "alerts",
            SearchDestination.NOTIFICATIONS,
        ),
        IndexableSetting(
            "notification_apps",
            "App notifications",
            "Allow or block notifications by app",
            "Notifications",
            "all apps alert",
            SearchDestination.NOTIFICATIONS,
        ),
        IndexableSetting(
            "privacy",
            "Privacy",
            "Microphone, camera, location and permissions",
            "Privacy",
            "permissions",
            SearchDestination.PRIVACY,
        ),
        IndexableSetting(
            "microphone",
            "Microphone",
        ...
        IndexableSetting(
            "location",
            "Location",
            "Allow apps and services to use location",
            "Privacy",
            "gps position",
            SearchDestination.PRIVACY_LOCATION,
        ),
        IndexableSetting(
            "location_settings",
            "Location",
            "Location access, ADAS location and recent access",
            "Location",
            "gps position adas provider",
            SearchDestination.LOCATION,
        ),
        IndexableSetting(
            "accessibility",
            "Accessibility",
            "Screen reader, captions and accessibility services",
            "Accessibility",
            "talkback captions spoken feedback",
            SearchDestination.ACCESSIBILITY,
        ),
        IndexableSetting(
            "assistant_voice",
            "Assistant & Voice",
            "Default assistant, screen context and voice input",
            "Assistant & Voice",
            "voice assistant screenshot screen context",
            SearchDestination.ASSISTANT_VOICE,
        ),
        IndexableSetting(
            "security",
            "Security",
            "Screen lock, credentials and device admins",
            "Security",
            "password pin pattern",
            SearchDestination.SECURITY,
        ),
        IndexableSetting(
            "screen_lock",
            "Screen lock",
            "None, Pattern, PIN or Password",
            "Security",
        ...
        IndexableSetting(
            "device_admin",
            "Device admin apps",
            "Manage active device administrators",
            "Security",
            "administrator policy",
            SearchDestination.DEVICE_ADMINS,
        ),
        IndexableSetting(
            "profiles",
            "Profiles & accounts",
            "Vehicle profiles, accounts and sync",
            "Profiles & accounts",
            "users account sync",
            SearchDestination.PROFILE_ACCOUNTS,
        ),
        IndexableSetting(
            "manage_profiles",
            "Manage other profiles",
            "Add or manage vehicle users",
            "Profiles & accounts",
            "users guest",
            SearchDestination.PROFILES,
        ),
        IndexableSetting(
            "auto_sync",
            "Automatically sync app data",
            "Control account synchronization",
            "Profiles & accounts",
            "accounts sync",
            SearchDestination.PROFILE_ACCOUNTS,
        ),
        ...
        IndexableSetting(
            "language",
            "Languages & input",
            "Language, keyboard and text to speech",
            "System",
            "keyboard tts input",
            SearchDestination.LANGUAGE_INPUT,
        ),
        ...
        IndexableSetting(
            "legal",
            "Legal information",
            "Open source licenses and legal notices",
            "System",
            "licenses",
            SearchDestination.LEGAL,
        ),
        IndexableSetting(
            "reset",
            "Reset options",
            "Network, app preferences and factory reset",
            "System",
            "factory reset",
            SearchDestination.RESET_OPTIONS,
        ),
    )
