...
private fun ProfileAccountsScreen(
    ...
    val focusSpec =
        profileAccountsRootFocusSpec(
            currentProfileId = settings.currentProfile?.id,
            accountFocusIds = settings.accounts.map(::profileAccountFocusId),
            canModifyAccounts = settings.canModifyAccounts,
        ...
        LazyColumn(
            ...
            settings.currentProfile?.let { profile ->
                item {
                    SettingsActionRow(
                        title = profile.name,
                        focusId = profileCurrentFocusId(profile.id),
...
/** Keeps root Profile & accounts handoff aligned with its conditional LazyColumn slots. */
internal fun profileAccountsRootFocusSpec(
    currentProfileId: Int?,
    accountFocusIds: List<String>,
    canModifyAccounts: Boolean,
    isWorking: Boolean,
) = settingsLazyFocusListSpec(
    buildList {
        add(SettingsLazyFocusListSlot())
        currentProfileId?.let { add(SettingsLazyFocusListSlot(profileCurrentFocusId(it))) }
        add(SettingsLazyFocusListSlot("profile-manage-other"))
        add(SettingsLazyFocusListSlot())
        add(
            SettingsLazyFocusListSlot(
                focusId = "profile-master-sync",
                isEnabled = canModifyAccounts && !isWorking,
            ),
        )
        if (accountFocusIds.isEmpty()) {
            add(SettingsLazyFocusListSlot())
        } else {
            accountFocusIds.forEach { add(SettingsLazyFocusListSlot(it)) }
        }
        add(
            SettingsLazyFocusListSlot(
                focusId = "profile-add-account",
                isEnabled = canModifyAccounts,
            ),
        )
    },
)

private fun profileCurrentFocusId(profileId: Int): String = "profile-current-$profileId"

private fun profileAccountFocusId(account: AccountSummary): String = "profile-account-${account.name}/${account.type}"
...
fun ProfileDetailsRoute(
    ...
    SettingsScaffold(
        ...
        destinationKey = "profile-details",
        onBack = onBack,
        actions = { RefreshAction(viewModel::refresh) },
    ) {
        SnackbarHost(hostState = snackbar)
...
fun AccountDetailsRoute(
    ...
    SettingsScaffold(
        ...
        destinationKey = "account-details",
        onBack = onBack,
        actions = { RefreshAction(viewModel::refresh) },
    ) {
        SnackbarHost(hostState = snackbar)
...
private fun AccountRow(
    ...
    title = account.name,
    focusId = profileAccountFocusId(account),
