...
internal class DisplayRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun setAdaptiveBrightness(enabled: Boolean) = platform.setAdaptiveBrightness(enabled)

        override suspend fun setThemeMode(mode: com.android.car.settings.feature.display.domain.ThemeMode) = platform.setThemeMode(mode)
        ...
        override suspend fun setUse24HourFormat(enabled: Boolean) = platform.setUse24HourFormat(enabled)

        override suspend fun setManualTime(epochMillis: Long) = platform.setManualTime(epochMillis)

        override suspend fun setManualTimeZone(timeZoneId: String) = platform.setManualTimeZone(timeZoneId)
    }
