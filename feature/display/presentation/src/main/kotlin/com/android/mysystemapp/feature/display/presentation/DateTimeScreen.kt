        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            items(matchingZones, key = { it }) { id ->
                ListItem(
                    ...
                    supportingContent = { Text(timeZoneSummary(id)) },
                    trailingContent = {
                        if (state.dateTime.timeZoneId == id) Text("Current")
                    },
                    modifier =
                        Modifier.clickable(enabled = !state.isWorking) {
                            viewModel.setManualTimeZone(id)
                        },
                )
            }
        }
